...
public final class XmlTreeStringPrinter {
    ...
    public static String printFileAst(File file) throws IOException, CheckstyleException {
        return printTree(XmlParser.parseFile(file));
    }

    /**
     * Prints full tree of the XmlAST.
     *
     * @param ast root XmlAST
     * @return Full tree
     */
    private static String printTree(XmlAST ast) {
     ...
     */
    public static String printBranch(XmlAST node) {
        final String result;
        if (node == null) {
            result = "";
        }
     ...
     * @return node info
     */
    private static String getNodeInfo(XmlAST node) {
        return XmlAstUtil.getTokenName(node.getType()) + " -> "
     ...
     */
    private static String getIndentation(XmlAST ast) {
